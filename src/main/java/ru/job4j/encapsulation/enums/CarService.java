...
public class CarService {
    public static void main(String[] args) {
        ...
        Order order = new Order(1, "Mercedes-Benz GLS", Status.FINISHED);
        System.out.println("Заказ-наряд №" + order.getNumber() + " на автомобиль " + order.getCar()
                + ", статус ремонта: " + order.getStatus().getInfo());

        Order order1 = new Order(1, "Mercedes-Benz GLS", Status.ACCEPTED);
        System.out.print(order1.getStatus().getInfo() + ": ");
        System.out.println(order1.getStatus().getMessage());
